...
def chebyshev_distance_heuristic(state):
    distance = 0
    for i in range(3):
        for j in range(3):
            value = state[i][j]
            if value != 0:
                ...
                distance = max(distance, max(abs(i - target_row), abs(j - target_col)))
    return distance
...
def solve_8_puzzle(initial_state):
    ...
    initial_node = PuzzleNode(state=initial_state, heuristic=chebyshev_distance_heuristic(initial_state))
    ...
    while priority_queue:
        ...
        for neighbor_state in get_neighbors(current_node):
            neighbor_node = PuzzleNode(
                ...
                heuristic=chebyshev_distance_heuristic(neighbor_state),
            )
